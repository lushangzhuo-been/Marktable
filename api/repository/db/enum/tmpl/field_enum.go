...
//字段level1（隐形字段，任务模块配套的字段，不可修改，不可删除）：status = blank、created_at = blank 、updated_at = blank、creator = blank
//字段level2（可以修改，但不能删除） : title = text_com
//字段level3（可以修改、也能删除） : 自定义字段

var FieldLevel1List = []model.FieldModel{
	{
		FieldKey:   "status",
		Name:       "状态",
		Desc:       "",
...
var FieldLevel2List = []model.FieldModel{
	{
		FieldKey:   "title",
		Name:       "标题",
		Desc:       "",
...
var FieldLevel3List = []model.FieldModel{
	{
