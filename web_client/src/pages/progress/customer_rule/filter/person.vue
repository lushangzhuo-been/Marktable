    ...
    >
        ...
        >
            ...
            <div slot="reference" class="detail">
                <div v-if="frontArr.length" class="tag-list">
                    <div
                        class="tag-item"
                        v-for="(tagItem, tagIndex) in frontArr"
                        :key="tagIndex"
...
export default {
    ...
    watch: {
        triggerListReshow: {
            handler(arr) {
                if (arr && arr.length) {
                    let info = _.find(arr, { field_key: this.fieldKey }).value;
                    this.submitArr = info;
...
// src\pages\progress\filter\filter_item\style.scss
@import "@/pages/progress/filter/filter_item/style.scss";
.default-text {
    color: #c0c4cc;
}
